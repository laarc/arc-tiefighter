...
setenv("defc", {_stash = true, macro = function (name, val)
  ...
  return({"do", {"ffi.cdef", {"quote", _e1}}, {"def", name, {"get", "ffi.C", {"quote", name}}}})
end})
ffi.cdef(" int usleep (unsigned int usecs) ")
usleep = ffi.C.usleep
function sleep(secs)
...
function response(data, code)
  return("HTTP/1.1 " .. code .. sep .. "Content-Type: text/html; charset=utf-8" .. sep .. "Content-Length: " .. _35(data) .. sep2 .. data)
end
...
function srv(f, ...)
  ...
  return(http.serve(port, function (req)
    prn(req)
    if f then
      return(f(req))
    end
...
function recv(req)
  return(motor.receive(req.fd))
end
setenv("echo", {_stash = true, macro = function (...)
  local l = unstash({...})
  local g = unique("g")
  local f = nil
  f = function (x)
    return({"let", g, x, {"prn", g}, {"stream.emit", "req", g}})
  end
  return(join({"do"}, map(f, l)))
end})
function run(port)
